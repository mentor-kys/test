...
### 학생
1. 배포된 주소로 접속 → 이름 입력 → 응시 가능한(마감 안 된) 시험지 목록에서 원하는 시험지 클릭
...
5. 제출 후에는 "제출 완료"만 표시됩니다 (채점은 자동으로 하지 않고, 멘토가 직접 확인합니다)

### 멘토
1. 시작 화면 맨 아래 "멘토 로그인" 클릭 (또는 주소 뒤에 `/mentor.html`)
2. 비밀번호 입력 후 "시험 문제 등록" / "시험지 확인" / "시험지 관리" 중 선택

**시험 문제 등록**: 시험지 이름을 적고, 아래 칸에 문제를 붙여넣습니다. 한 줄에 문제 하나씩 입력하세요 (줄바꿈 = 문제 구분). 정답은 따로 입력하지 않습니다 (채점은 멘토가 직접).
```
사과를 영어로 쓰시오.
바나나를 영어로 쓰시오.
```
등록한 시험지는 바로 학생 시작 화면에 응시 가능한 목록으로 나타납니다.

**시험지 확인**: 응시한 학생 이름 목록 → 이름을 누르면 그 학생이 본 시험지 목록 → 시험지를 누르면 그때 입력한 답안(문제 순서대로, 번호만)을 확인할 수 있습니다. 채점은 이 답안을 보고 멘토가 직접 판단합니다.

**시험지 관리**: 모든 시험지의 진행중/마감됨 상태를 보고, "마감하기"/"다시 열기"로 즉시 전환할 수 있습니다. 마감한 시험지는 학생 목록에서 사라집니다. 마감된 시험지에는 "삭제" 버튼이 나타나며, 삭제하면 그 시험지와 문제, 그동안 제출된 응시 결과가 모두 함께 삭제됩니다 (되돌릴 수 없음, 확인 창 있음).
